...
function isColliding(position) {
    ...
    if (mapPos.ox) {
        checkspots.push({x: Math.floor(mapPos.ox), y: Math.floor(mapPos.y), z: Math.floor(mapPos.z)});
    }
    if (mapPos.oz) {
        checkspots.push({x: Math.floor(mapPos.x), y: Math.floor(mapPos.y), z: Math.floor(mapPos.oz)});
    }
    if (mapPos.ox && mapPos.oz) {
        checkspots.push({x: Math.floor(mapPos.ox), y: Math.floor(mapPos.y), z: Math.floor(mapPos.oz)});
    }

    checkspots.push({x: Math.floor(mapPos.x), y: Math.floor(mapPos.y) + 1, z: Math.floor(mapPos.z)});
    if (mapPos.ox) {
        checkspots.push({x: Math.floor(mapPos.ox), y: Math.floor(mapPos.y) + 1, z: Math.floor(mapPos.z)});
    }
    if (mapPos.oz) {
        checkspots.push({x: Math.floor(mapPos.x), y: Math.floor(mapPos.y) + 1, z: Math.floor(mapPos.oz)});
    }
    if (mapPos.ox && mapPos.oz) {
        checkspots.push({x: Math.floor(mapPos.ox), y: Math.floor(mapPos.y) + 1, z: Math.floor(mapPos.oz)});
    }
...
function animate() {
    ...
    if (controls.isLocked === true) {
        ...
        if (sprint && (moveForward || moveBackward)) {
            ...
            velocity.z -= direction.z * 350.0 * delta;
        }
        ...
        if (isColliding(originalPosition)) {
            ...
            controls.getObject().position.y = originalPosition.y -0.5; // THIS STOPS JUMPING THROUGH CEILINGS
...
socket.on("play bullet hit sound", () => {
    var audio = new Audio('/sounds/guns/bullet_hit');
    audio.volume = 0.7
    audio.play();
})
...
socket.on("play bullet sound", function (shooter_pos) {
...

    {
        ...
        // Set cube position to player's camera position
        cube.position.set(shooter_pos.x, shooter_pos.y, shooter_pos.z)
        ...
        const loader = new THREE.AudioLoader().load('/bullet_sound', (buffer) => {
            ...
            bullet_sound.setRefDistance(200)
            bullet_sound.play()
        })

        // Set temporary cube name, This enables deleting object later
        const temp_name = `bullet_sound_${Date.now()}`
        cube.name = temp_name;

        // Add cube to scene
        cube.add(bullet_sound)
        scene.add(cube)

        // Remove object after timeout
        setTimeout(() => {
            const selectedObject = scene.getObjectByName(temp_name);
...
async function sendDataToServer() {
    while ("Vincent" > "Michael") {
        ...
        socket.emit("player position", {
            ...
            y: controls.getObject().position.y - 14,
